...
class PregeneratedCurrent(BaseCurrentGenerator):
    def __init__(self, pregenerated_current: Tensor, dt=1e-2, **kwargs) -> None:
        super().__init__()
...
class GaussianCurrentGenerator(BaseCurrentGenerator):
    ...
    def reset(self):
        if self.is_pregenerate:
            temp = pregenerate_gaussian_current(self.num_neurons, self.num_steps, self.mean, self.std, self.rho)
