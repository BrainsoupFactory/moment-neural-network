...
def gen_lookup():
    ...
    # show input stats collected from DTB model
    
    file = 'projects/dtb/data/V1_SNN_weight_totalinputrate_unweighted.mat'  
    dat = loadmat(file)
    #print(list(dat))
    #'exc_input_mean_forEpopu', 'local_exc_input_mean_forEpopu', 'xregion_exc_input_mean_forEpopu', 
    #'inh_input_mean_forEpopu', 'exc_input_mean_forIpopu', 'inh_input_mean_forIpopu', 
    #'local_w_EE_after_scaling', 'local_w_EI_after_scaling', 'local_w_IE_after_scaling', 
    #'local_w_II_after_scaling', 'xregion_gain_for_E_popu_after_scaling'
    keys = list(dat)[3:]
    renamed_keys = [
        'total E-to-E input rate', 'local E-to-E input rate', 'xregion E-to-E input rate', 
        'local I-to-E input rate', 'total E-to-I input rate', 'local I-to-I input rate', 
        'local w_EE', 'local w_EI', 'local w_IE', 
        'local w_II', 'xregion w_EE'
        ]
    ...
    for i in range(len(keys)):
        plt.subplot(3,4,i+1)
        plt.hist( dat[keys[i]].flatten(), 50)
        plt.title(renamed_keys[i])
    plt.tight_layout()
    
    # load single neuron parameters
    exc_neuron_config = loadmat('projects/dtb/data/config_for_Moment_exc_activation.mat')
    inh_neuron_config = loadmat('projects/dtb/data/config_for_Moment_inh_activation.mat')
    
    pprint(exc_neuron_config)
    pprint(inh_neuron_config)
